...
const btnPrimary = { fontSize: 13, padding: "7px 14px", borderRadius: 8, border: "none",
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 24px" }}>
            ...
            {d.contract_file_url && (
              <div style={{ gridColumn: "1/-1" }}>
                <p style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase",
                  letterSpacing: "0.06em", margin: "0 0 5px" }}>File</p>
                <a href={d.contract_file_url} target="_blank" rel="noreferrer"
                  style={{ fontSize: 13, color: "#6366f1" }}>📎 Open</a>
              </div>
            )}
          </div>
        </>
    ...
    <Modal title={initial ? `Edit Contract #${initial.id}` : "New Lease Contract"} onClose={onClose} maxWidth={620}>
      ...
      <Row2>
        ...
        <Field label="End Date" required>
          <input style={{ ...inputSt, borderColor: dateError ? "#f87171" : "#d1d5db" }}
      ...
      {form.start_date && form.end_date && !dateError && (
        <div style={{ display: "flex", alignItems: "center", background: "#f0fdf4",
          border: "1px solid #a7f3d0", borderRadius: 10, padding: "10px 18px", marginBottom: 12 }}>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 4 }}>
        ...
        <button className="btn btn--primary" onClick={handleSubmit}
          disabled={saving || !!dateError} style={{ opacity: dateError ? 0.55 : 1 }}>
  ...
  return (
    <Modal title={`Ndrysho Statusin — #${contract.id}`} onClose={onClose}>
      <p style={{ fontSize: 13, color: "#64748b", marginBottom: 16 }}>
        Aktual: <Badge label={contract.status} />
      </p>
      ...
      <div style={{ background: isCancel ? "#fef2f2" : "#f0fdf4",
...
export default function AdminContracts() {
  // ── Filter state ───────────────────────────────────────────────────────────
  // filterMode: "default" | "client" | "agent" | "property" | "expiring"
  ...
  // Applied filter values (set when Load is clicked or expiring toggled)
  ...
  // ── Fetch — switches endpoint based on active filter ───────────────────────
  // fetchContracts — called directly from useEffect, no useCallback wrapper
  // to avoid stale closure issues with boolean deps like expiring
  ...
  const applyClientFilter = () => {
    ...
    if (!clientInput.trim() || isNaN(id) || id <= 0) {
      notify("Shkruaj Client ID të vlefshëm", "error"); return;
    }
    setExpiring(false);
    setAgentInput(""); setPropertyInput("");
  ...
  const applyAgentFilter = () => {
    ...
    if (!agentInput.trim() || isNaN(id) || id <= 0) {
      notify("Shkruaj Agent ID të vlefshëm", "error"); return;
    }
    setExpiring(false);
    setClientInput(""); setPropertyInput("");
  ...
  const applyPropertyFilter = () => {
    ...
    if (!propertyInput.trim() || isNaN(id) || id <= 0) {
      notify("Shkruaj Property ID të vlefshëm", "error"); return;
    }
    setExpiring(false);
    setClientInput(""); setAgentInput("");
    setActiveClientId(null); setActiveAgentId(null);
    setActivePropertyId(id);
    setFilterMode("property");
    setPage(0);
  };

  const toggleExpiring = () => {
    if (!expiring) {
      // Turn ON expiring — clear all other filters first
      ...
      setPage(0);
      setExpiring(true);  // set last so useEffect fires with clean state
    } else {
      // Turn OFF expiring
      setExpiring(false);
  ...
  const activeFilterLabel = expiring ? "⏰ Expiring soon (30 days)"
    ...
    : filterMode === "property" ? `Property #${activePropertyId}`
    : null;
    ...
    <MainLayout role="admin">
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: 140 }}>
          <h1 style={{ fontSize: 20, fontWeight: 500, margin: 0 }}>Lease Contracts</h1>
          <p style={{ fontSize: 13, color: "#94a3b8", margin: "2px 0 0" }}>
            Admin view — shiko dhe filtro të gjitha kontratat e qirasë
          </p>
        </div>

        {/* Toggle Filters button — same as AllProperties */}
        <button
          onClick={() => setShowFilters(v => !v)}
          style={{
            fontSize: 12, padding: "6px 12px", borderRadius: 8,
            border: `0.5px solid ${(filterMode !== "default" || expiring) ? "#6366f1" : "#e8edf4"}`,
            background: (filterMode !== "default" || expiring) ? "#eef2ff" : showFilters ? "#f8fafc" : "transparent",
            color: (filterMode !== "default" || expiring) ? "#6366f1" : "#64748b",
            cursor: "pointer", fontWeight: (filterMode !== "default" || expiring) ? 500 : 400,
          }}
        >
          🔍 Filters {(filterMode !== "default" || expiring) ? "●" : ""}
        </button>

        {/* Search — Contract ID ose Property ID */}
        <div style={{
          display: "flex", alignItems: "center", gap: 8,
          background: "#f8fafc", border: "0.5px solid #e8edf4",
          borderRadius: 8, padding: "6px 12px", flex: 1, maxWidth: 340, minWidth: 180,
        }}>
          ...
          {search && (
            <button onClick={() => setSearch("")}
              style={{ border: "none", background: "transparent", cursor: "pointer",
                fontSize: 17, lineHeight: 1, padding: 0, color: "#94a3b8" }}>×</button>
          )}
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        {[
          ...
          <div key={s.label} style={{ background: "#fff", borderRadius: 12,
            border: "1px solid #e8edf4", padding: "12px 18px", flex: "1 1 90px", minWidth: 80 }}>
            <p style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase",
              letterSpacing: "0.06em", margin: "0 0 4px" }}>{s.label}</p>
            <p style={{ fontSize: 22, fontWeight: 700, color: s.color, margin: 0,
              letterSpacing: "-0.02em" }}>{s.val}</p>
          </div>
        ))}
      </div>

      {/* ── FILTERS BAR — toggle like AllProperties ── */}
      {showFilters && (
      <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e8edf4",
        ...
        <p style={{ fontSize: 12, fontWeight: 600, color: "#374151", margin: "0 0 12px",
          textTransform: "uppercase", letterSpacing: "0.06em" }}>Filtrat</p>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
          ...
          <div>
            <label style={{ fontSize: 11, fontWeight: 500, color: "#64748b",
              display: "block", marginBottom: 5 }}>Client ID</label>
            <div style={{ display: "flex", gap: 4 }}>
              <input type="number" value={clientInput}
                onChange={e => setClientInput(e.target.value)}
                onKeyDown={e => e.key === "Enter" && applyClientFilter()}
                placeholder="ex: 3"
                style={{ width: 90, padding: "7px 10px", fontSize: 13,
                  border: `1px solid ${filterMode === "client" ? "#6366f1" : "#d1d5db"}`,
                  borderRadius: 8, outline: "none", color: "#0f172a",
                  background: filterMode === "client" ? "#eef2ff" : "#fff" }} />
          ...
          <div>
            <label style={{ fontSize: 11, fontWeight: 500, color: "#64748b",
              display: "block", marginBottom: 5 }}>Agent ID</label>
            <div style={{ display: "flex", gap: 4 }}>
              <input type="number" value={agentInput}
                onChange={e => setAgentInput(e.target.value)}
                onKeyDown={e => e.key === "Enter" && applyAgentFilter()}
                placeholder="ex: 2"
                style={{ width: 90, padding: "7px 10px", fontSize: 13,
                  border: `1px solid ${filterMode === "agent" ? "#6366f1" : "#d1d5db"}`,
                  borderRadius: 8, outline: "none", color: "#0f172a",
                  background: filterMode === "agent" ? "#eef2ff" : "#fff" }} />
              <button onClick={applyAgentFilter}
                style={{ padding: "7px 10px", borderRadius: 8, border: "1px solid #d1d5db",
                  background: "#fff", color: "#374151", cursor: "pointer", fontSize: 12,
                  fontWeight: 500 }}>Load</button>
            </div>
          ...
          <div>
            <label style={{ fontSize: 11, fontWeight: 500, color: "#64748b",
              display: "block", marginBottom: 5 }}>Property ID</label>
            <div style={{ display: "flex", gap: 4 }}>
              <input type="number" value={propertyInput}
                onChange={e => setPropertyInput(e.target.value)}
                onKeyDown={e => e.key === "Enter" && applyPropertyFilter()}
                placeholder="ex: 1"
                style={{ width: 90, padding: "7px 10px", fontSize: 13,
                  border: `1px solid ${filterMode === "property" ? "#6366f1" : "#d1d5db"}`,
                  borderRadius: 8, outline: "none", color: "#0f172a",
                  background: filterMode === "property" ? "#eef2ff" : "#fff" }} />
              <button onClick={applyPropertyFilter}
                style={{ padding: "7px 10px", borderRadius: 8, border: "1px solid #d1d5db",
                  background: "#fff", color: "#374151", cursor: "pointer", fontSize: 12,
                  fontWeight: 500 }}>Load</button>
            </div>
          ...
          <div>
            <label style={{ fontSize: 11, fontWeight: 500, color: "#64748b",
              display: "block", marginBottom: 5 }}>Expiring Soon</label>
            <button onClick={toggleExpiring} style={{
              padding: "7px 14px", borderRadius: 8, fontSize: 13, fontWeight: 500,
              cursor: "pointer", transition: "all .15s",
              border: expiring ? "none" : "1px solid #d1d5db",
              background: expiring ? "#f59e0b" : "#fff",
              color: expiring ? "#fff" : "#374151",
            }}>
          ...
          {/* Clear */}
          {(filterMode !== "default" || expiring) && (
            <div style={{ marginLeft: "auto" }}>
              <label style={{ fontSize: 11, color: "transparent", display: "block", marginBottom: 5 }}>.</label>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end", marginTop: 14,
          ...
          <div>
            <label style={{ fontSize: 11, fontWeight: 500, color: "#64748b",
              display: "block", marginBottom: 5 }}>Rent min (€)</label>
            <input type="number" value={minRent} onChange={e => setMinRent(e.target.value)}
              placeholder="ex: 300"
              style={{ width: 110, padding: "7px 10px", fontSize: 13,
                border: "1px solid #d1d5db", borderRadius: 8, outline: "none", color: "#0f172a" }} />
          </div>
          <div>
            <label style={{ fontSize: 11, fontWeight: 500, color: "#64748b",
              display: "block", marginBottom: 5 }}>Rent max (€)</label>
          ...
          {(minRent || maxRent) && (
            <button onClick={() => { setMinRent(""); setMaxRent(""); }}
        ...
        {activeFilterLabel && (
          <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 12, color: "#64748b" }}>Filtri aktiv:</span>
            <span style={{ background: "#eef2ff", color: "#6366f1", fontSize: 12,
      ...
      <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e8edf4", overflow: "hidden" }}>
        {/* Toolbar */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
          padding: "14px 20px", borderBottom: "1px solid #e8edf4", flexWrap: "wrap", gap: 10 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <select value={statusF} onChange={e => setStatusF(e.target.value)}
              style={{ ...selectSt, width: 170, height: 34, padding: "0 10px" }}>
              <option value="">All statuses</option>
              {LEASE_STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
            <span style={{ fontSize: 12, color: "#94a3b8" }}>
              {displayed.length} kontrata
            </span>
          </div>
        ...
        {loading ? <Loader /> : displayed.length === 0 ? (
          <Empty icon="📋"
            text={filterMode !== "default" || expiring
              ? "Nuk u gjet asnjë kontratë për filtrin e zgjedhur."
              : "Nuk ka lease contracts."} />
        ) : (
          <>
                  ...
                  {displayed.map(c => {
                    ...
                    return (
                      <tr key={c.id}
                        ...
                        <td style={{ ...TD, color: nearExpiry ? "#d97706" : "#64748b", fontWeight: nearExpiry ? 600 : 400 }}>
                          {fmtDate(c.end_date)}
                          {nearExpiry && <span style={{ fontSize: 10, marginLeft: 4, color: "#d97706" }}>{days}d</span>}
                        </td>
      ...
      {createOpen   && <ContractFormModal onClose={() => setCreateOpen(false)}
        onSuccess={() => { setCreateOpen(false); refetch(); notify("Kontrata u krijua"); }} notify={notify} />}
      {editTarget   && <ContractFormModal initial={editTarget} onClose={() => setEditTarget(null)}
        onSuccess={() => { setEditTarget(null); refetch(); notify("Kontrata u ndryshua"); }} notify={notify} />}
      {detailTarget && <ContractDetailModal contract={detailTarget} onClose={() => setDetail(null)} />}
      {statusTarget && <StatusModal contract={statusTarget} onClose={() => setStatusTgt(null)}
