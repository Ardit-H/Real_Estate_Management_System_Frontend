...
const fmtMoney = (v, cur = "EUR") => v != null ? `€${Number(v).toLocaleString("de-DE")} ${cur}` : "—";
...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{ background:s.bg, color:s.color, padding:"3px 10px", borderRadius:20, fontSize:11.5, fontWeight:600 }}>
      {status?.replace("_"," ")}
    </span>
  );
}
...
function ContractModal({ initial, onClose, onSuccess, notify }) {
  ...
  const handleSubmit = async () => {
    ...
    if (form.end_date <= form.start_date) {
      notify("Data e përfundimit duhet të jetë pas datës fillimit", "error"); return;
    }
    setSaving(true);
    try {
      const payload = {
    ...
    <Modal title={initial ? `Edit Kontratë #${initial.id}` : "New Lease Contract"} onClose={onClose} wide>
      <Row2>
        ...
        <Field label="Listing ID">
          <input className="form-input" type="number" value={form.listing_id} onChange={e => set("listing_id", e.target.value)} placeholder="(opcional)" />
        </Field>
      </Row2>
      <Field label="Client ID" required>
        <input className="form-input" type="number" value={form.client_id} onChange={e => set("client_id", e.target.value)} disabled={!!initial} placeholder="ID e klientit" />
      </Field>
      <Row2>
      ...
      <Row2>
        <Field label="Qiraja mujore" required>
          <input className="form-input" type="number" value={form.rent} onChange={e => set("rent", e.target.value)} placeholder="450" />
        </Field>
      ...
      <Row2>
        ...
        <Field label="URL Kontratë">
          <input className="form-input" value={form.contract_file_url} onChange={e => set("contract_file_url", e.target.value)} placeholder="https://..." />
        </Field>
      </Row2>
...
function StatusModal({ contract, onClose, onSuccess, notify }) {
  ...
  return (
    <Modal title={`Ndrysho statusin — Kontratë #${contract.id}`} onClose={onClose}>
      <p style={{ fontSize:13.5, color:"#475569", marginBottom:16 }}>Statusi aktual: <StatusBadge status={contract.status} /></p>
      ...
      <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}>
        <button className="btn btn--secondary" onClick={onClose}>Anulo</button>
        <button className={`btn ${status === "CANCELLED" ? "btn--danger" : "btn--primary"}`} onClick={handleSubmit} disabled={saving}>
...
// ─── Contract Detail Modal — me buton "Shiko Pagesat" ────────────────────────
function ContractDetailModal({ contract, onClose, onEdit, onStatusChange, onGoToPayments }) {
  ...
  return (
    <Modal title={`Kontratë #${contract.id}`} onClose={onClose} wide>
      {expiring && (
        <div style={{ background:"#fffbeb", border:"1px solid #fde68a", borderRadius:8, padding:"10px 14px", marginBottom:18, fontSize:13, color:"#92400e" }}>
      ...
      {/* ── Buton kryesor: shko te pagesat direkt ── */}
      <div style={{ background:"#f0f4ff", border:"1px solid #c7d7fe", borderRadius:10, padding:"14px 16px", marginBottom:16, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div>
          <p style={{ fontWeight:600, fontSize:13.5, color:"#3730a3", marginBottom:2 }}>💳 Pagesat e kësaj kontrate</p>
          <p style={{ fontSize:12.5, color:"#6366f1" }}>Shiko, krijo dhe menaxho pagesat e qirasë</p>
        </div>
        <button className="btn btn--primary" onClick={() => onGoToPayments(contract.id)}
          style={{ whiteSpace:"nowrap", flexShrink:0 }}>
          Shiko Pagesat →
        </button>
      </div>

      <div style={{ display:"flex", gap:10, justifyContent:"flex-end", borderTop:"1px solid #e8edf4", paddingTop:18 }}>
        {contract.status !== "ENDED" && contract.status !== "CANCELLED" && (
          <>
            <button className="btn btn--secondary" onClick={() => onEdit(contract)}>Edit</button>
            <button className="btn btn--primary" onClick={() => onStatusChange(contract)}>Ndrysho statusin</button>
          </>
        )}
      </div>
    </Modal>
...
export default function AgentContracts() {
  ...
  // Merr propertyId nga navigation state nëse vjen nga Rentals
  const fromPropertyId = location.state?.propertyId;
  ...
  // Nëse vjen nga faqja e Rentals me propertyId, hap direkt create modal
  useEffect(() => {
    if (fromPropertyId) setCreateOpen(true);
  }, [fromPropertyId]);
  ...
  // ── Navigim te Payments me contractId parafilluar ──────────────
  const goToPayments = (contractId) => {
    navigate("/agent/payments", { state: { contractId } });
  };
    ...
    <MainLayout role="agent">
      ...
      <div className="card">
        <div className="card__header">
          ...
          {contracts.length > 0 && (
            <span style={{ background:"#eef2ff", color:"#6366f1", padding:"3px 10px", borderRadius:20, fontSize:12, fontWeight:500 }}>{contracts.length}</span>
          )}
        </div>

        {loading ? <Loader /> : contracts.length === 0 ? (
          <EmptyState icon="📄" text={tab==="expiring" ? "Nuk ka kontrata që skadojnë së shpejti" : "Nuk ka kontrata"} />
        ) : (
          <>
                  ...
                  {contracts.map(c => {
                    ...
                    return (
                      <tr key={c.id}>
                        ...
                        <td><span style={{ background:"#eef2ff", color:"#6366f1", padding:"2px 7px", borderRadius:20, fontSize:12 }}>#{c.property_id}</span></td>
                        <td><span style={{ background:"#f0f9ff", color:"#0ea5e9", padding:"2px 7px", borderRadius:20, fontSize:12 }}>#{c.client_id}</span></td>
                        ...
                        <td>
                          {days !== null && c.status === "ACTIVE" ? (
                            <span style={{ fontWeight:600, fontSize:12.5, color:expiring ? "#d97706" : "#64748b" }}>
                        ...
                        <td>
                          <div style={{ display:"flex", gap:5 }}>
                            <button className="btn btn--ghost btn--sm" onClick={() => setDetailTarget(c)}>View</button>
                            {/* ── Buton i drejtpërdrejtë te Payments ── */}
                            <button className="btn btn--secondary btn--sm"
                              style={{ background:"#eef2ff", color:"#6366f1", border:"1px solid #c7d7fe" }}
                              title={`Shiko pagesat e kontratës #${c.id}`}
      ...
      {createOpen && (
        <ContractModal
          initial={fromPropertyId ? { property_id: fromPropertyId } : undefined}
          onClose={() => setCreateOpen(false)}
          onSuccess={() => handleSuccess("Kontrata u krijua")}
          notify={notify}
        />
      )}
      {editTarget    && <ContractModal initial={editTarget} onClose={() => setEditTarget(null)} onSuccess={() => handleSuccess("Kontrata u ndryshua")} notify={notify} />}
