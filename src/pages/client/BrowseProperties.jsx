...
function PropertyDetailModal({ propertyId, onClose, onApply }) {
  ...
  const isRent  = property?.listingType === "RENT" || property?.listing_type === "RENT"
               || property?.listingType === "BOTH"  || property?.listing_type === "BOTH";
            ...
            <>
              ...
              <div style={{ position:"relative", height:"320px", background:"#1a1a14", borderRadius:"18px 18px 0 0", overflow:"hidden" }}>
                ...
                <button
                  ...
                  style={{ position:"absolute", bottom:"14px", right:"14px", background:"rgba(0,0,0,0.5)", border:"none", borderRadius:"8px", color:"#fff", padding:"6px 10px", cursor:"pointer", display:"flex", alignItems:"center", gap:"5px", fontSize:"12px", backdropFilter:"blur(4px)" }}
                >
                  ...
                  <div style={{ display:"flex", gap:"10px", flexWrap:"wrap" }}>
                    ...
                    {isRent && (property.status === "AVAILABLE" || !property.status) && (
                      <button
                        ...
                        style={{
                          display:"flex", alignItems:"center", gap:"7px",
                          background:"#c9a84c", color:"#1f1f1f",
                          border:"none", borderRadius:"10px",
                          padding:"10px 18px", fontSize:"13.5px", fontWeight:700,
                          cursor:"pointer", fontFamily:"inherit",
                        }}
      ...
      <div style={{ width:"100%", maxWidth:540, background:"#fff", borderRadius:16, boxShadow:"0 20px 60px rgba(15,23,42,0.2)", maxHeight:"90vh", overflowY:"auto", animation:"fadeUp .2s ease" }}>
        ...
        <div style={{ padding:"20px 24px" }}>
          <div style={{ marginBottom:18 }}>
            ...
            {loadingL ? (
              <div style={{ padding:"14px", textAlign:"center", color:"#94a3b8", fontSize:13 }}>Duke ngarkuar listings...</div>
            ) : listings.length===0 ? (
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {listings.map(l => (
                  <div key={l.id} onClick={()=>setSelectedL(l)}
                    style={{ padding:"12px 14px", borderRadius:10, cursor:"pointer", border:`2px solid ${selectedL?.id===l.id?"#6366f1":"#e2e8f0"}`, background:selectedL?.id===l.id?"#eef2ff":"#f8fafc", transition:"all 0.15s" }}
                  >
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                      ...
                      {selectedL?.id===l.id && (
                        <div style={{ width:20, height:20, borderRadius:"50%", background:"#6366f1", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontSize:12, fontWeight:700, flexShrink:0 }}>✓</div>
                      )}
                    </div>
                  </div>
                ))}
            ...
            <>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:14 }}>
                <div>
                  ...
                  <input className="form-input" type="number" min="0" placeholder="p.sh. 1500"
                    value={form.income} onChange={e=>setForm(f=>({...f,income:e.target.value}))}/>
                </div>
                <div>
                  <label style={S.label}>Data e hyrjes</label>
                  <input className="form-input" type="date"
                    value={form.move_in_date} onChange={e=>setForm(f=>({...f,move_in_date:e.target.value}))}/>
                </div>
              </div>
              <div style={{ marginBottom:20 }}>
                <label style={S.label}>Mesazhi (opcional)</label>
                <textarea rows={3} placeholder="Prezantohuni dhe shpjegoni pse jeni kandidati ideal..."
